...
async function loadRefreshData(x) {
   ...
   page = x || 1; // x or 1(x = null, undefined, 0)
	...
	try {
        const response = await $.get("/sample/list/refresh", {page, keyword, type});
        ...
        const html = boardList.map(({bno, title, regDate, nickname, replyCount, count}) =>
            `<div>
               ...
               <div class="date item_4">${moment(regDate).format('YYYY/MM/DD HH:mm:ss')}</div>
...
function loadReadData(bno){
    ...
    $.getJSON("/sample/list/" + bno, function(result){
        ...
        const str = `<div class="titlebox box4">
       						...
       						<div class="boardviewbox box4">
       							<div class="boardview box5">
       								<div class="title item" name="title">${board.title}</div>
								        ...
								        <div class="content item" name="content">${board.content}</div>
								    </div>
       							</div>
								<div class="boardviewbox box4">
            ...
            repliesListRE.html(
					`	<div class="ct_wr_debox">
            	        	...
            	        	<div class="commentdetailbox">
            	        		...
            	        		<input type="hidden" id="showReplyContent${reply.rno}" value="${reply.content}">
            	        		<div id="modifyContentDiv${reply.rno}" class="replyContentBox ${reply.rno} modifyContentDiv${reply.rno}"></div>
            	        	</div>
            	        </div>
